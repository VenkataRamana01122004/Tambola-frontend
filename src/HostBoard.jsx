...
export default function HostBoard() {
  ...
  setPlayers(prev =>
    prev.filter(p => p.playerCode !== playerCode)
  );
  ...
  if (claimType === "FULL_HOUSE") {
    // alert(`🏁 FULL HOUSE! Winner: ${winner}. Game ended.`);
      toast.success(`🏁 FULL HOUSE! Winner: ${winner}. Game ended🏆`, {
  duration: 4000,
    ...
    <div className="chat-modal">
      ...
      <div className="chat-messages">
        {messages.map((m, i) => (
          <div
            key={i}
            className={`chat-msg ${m.user === "HOST" ? "host-msg" : ""}`}
          >
  ...
  {roomCode && <button className="chat-toggle-btn" onClick={() => setChatOpen(v => !v)}>
    💬 Chat
  </button>}
</div>
...
>
  ...
  >
    ...
    🤖 Auto Generate
        ...
        {roomCode &&<div style={sideBySideContainer}>
          {/* Players List */}
...
<input
  ...
  onChange={(e) => {
    ...
    setPlayers(prev =>
      prev.map(player =>
        player.playerCode === p.playerCode
    ...
    // 2️⃣ Notify server
    socket.emit("toggle_player_auto_mark", {
      roomCode,
      playerCode: p.playerCode,
      allowed,
    });
...
>
  {assigned[p.playerCode] ? "✔️" : "⏳"}
      ...
      {roomCode && <div style={rightPanelStyle}>
        <h3
          style={{
            margin: "0 0 12px",
            color: colors.textPrimary,
